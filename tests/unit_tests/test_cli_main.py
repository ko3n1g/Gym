...
class TestEnvInitFlags:
    ...
    def test_scaffold_options_are_rejected_for_legacy_init(self, monkeypatch: MonkeyPatch) -> None:
        monkeypatch.setattr(
            sys,
            "argv",
            ["gym", "env", "init", "--resources-server", "sample", "--profile", "stock-loop"],
        )
        with pytest.raises(SystemExit):
            main()
...
class TestAssetSelectors:
    ...
    def test_nested_manifest_name_resolves_through_catalog(self, monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
        config_path = tmp_path / "benchmarks/suite/example/config.yaml"
        monkeypatch.setattr(
            "nemo_gym.registry.resolve_catalog_entry",
            lambda name, kind: types.SimpleNamespace(
                config_path=config_path,
                manifest_path=config_path.with_name("manifest.yaml"),
            ),
        )

        _, overrides = _dispatch_for(monkeypatch, ["env", "start", "--benchmark", "suite/example"])

        assert overrides == [f"+config_paths=[{config_path.resolve()}]"]
