...
class VLLMModelConfig(BaseResponsesAPIModelConfig):
    ...
    # Sampling params this server puts on every request it sends to the engine, replacing what the
    # caller sent. On-policy training requires generation to use the sampling distribution the
    # policy is optimized under, and a caller outside the training loop has no way to know it.
    #
    # Two cases, both observed. Most often the parameter is absent: request handling is set-or-omit
    # throughout, so a field the caller did not set is not forwarded, and a caller built for
    # interactive serving has no reason to send sampling params. The outbound body then carries no
    # temperature or top_p and the engine applies a default of its own. Less often the parameter is
    # present and wrong: a caller that sets its own temperature on auxiliary calls, such as context
    # compression or title generation, sends a value nobody configured.
    #
    ...
    # that does not generates off-policy and reports nothing. Because both cases are real, these
    # values replace rather than only fill in.
    #
    # Read from config only, never from a request, so the server and not the caller decides them.
    # Applied at every site that builds a request for the engine, since a pin that covers some
    # endpoints and not others is off-policy while reporting that sampling is pinned.
    #
    # Unset means no pin.
...
class VLLMModel(SimpleResponsesAPIModel):
    ...
    def _post_init(self) -> None:
        # Say which sampling profile this server will enforce, if any. The pin is invisible from
        # outside otherwise: a run that meant to enforce one but was never wired looks exactly like
        # a run that meant not to.
        if self.config.sampling_overrides:
            ...
            LOG.info(
                "`%s` does not pin sampling; requests reach the engine as the caller sent them.", self.config.name
            )
