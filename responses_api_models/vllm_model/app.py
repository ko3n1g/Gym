...
class VLLMModelConfig(BaseResponsesAPIModelConfig):
    ...
    #
    # The common case is an absent parameter rather than a conflicting one. A caller need not send
    # sampling params at all, and the converters forward a field only when it was set, so the
    # outbound body can carry no temperature or top_p and the engine applies a default of its own
    # that has no relation to the configured one. Replacing rather than filling in covers the other
    # case, a caller that sends values it chose itself.
    #
