...
def scaffold_environment(
    *,
    ...
    remains the sole source of runtime wiring. Non-stock templates delegate to
    existing stock behavior until their generated extension point is replaced.
    """

    environment_kind = _parse_kind(kind)
    integration_profile = _parse_profile(profile)
    _validate_name(name, requires_python_module=reuse_verifier is None)
    scaffold_root = _resolve_root(root)

    reused = None
    if reuse_verifier is not None:
        if integration_profile != IntegrationProfile.STOCK_LOOP:
    ...
    files = _render_files(scaffold_root, composition)
...
def _composition(
    *,
    ...
    custom_agent = profile in {IntegrationProfile.MEASURED_LOOP, IntegrationProfile.EXTERNAL_LOOP}
...
def _render_files(root: Path, composition: _Composition) -> dict[Path, str]:
    ...
    if composition.reused_verifier is None:
        ...
        files.update(_resources_server_files(root, resource_dir, composition))
    if composition.profile in {IntegrationProfile.MEASURED_LOOP, IntegrationProfile.EXTERNAL_LOOP}:
...
def _resources_server_files(root: Path, directory: Path, composition: _Composition) -> dict[Path, str]:
    module_name = composition.module_name
    return {
        directory / "__init__.py": _license_header(),
        directory / "README.md": f"# {module_name} resources server\n\n{_TODO}: Document the verifier.\n",
        directory / "app.py": _resources_server_app(module_name),
        directory / "configs" / f"{module_name}.yaml": _resources_server_config(composition),
        directory / "requirements.txt": _requirements(root),
        directory / "tests" / "__init__.py": _license_header(),
        directory / "tests" / "test_app.py": _resources_server_test(module_name),
        directory / "tests" / "verifier_cases.jsonl": _verifier_cases(),
    }


def _resources_server_config(composition: _Composition) -> str:
...
def _agent_files(root: Path, directory: Path, composition: _Composition) -> dict[Path, str]:
    ...
    if composition.profile == IntegrationProfile.EXTERNAL_LOOP:
        instruction += " The generated run() delegates to SimpleAgent until it is replaced."
    return {
        directory / "__init__.py": _license_header(),
    ...
    if profile == IntegrationProfile.MEASURED_LOOP:
        ...
        class {class_name}Agent(SimpleAgent):
            async def responses(
                ...
                # TODO: Implement the measured agent strategy.
        ...
        class {class_name}Agent(SimpleAgent):
            async def run(self, request: Request, body: SimpleAgentRunRequest) -> SimpleAgentVerifyResponse:
                # Runnable scaffold fallback; replace with external episode delegation.
...
def _requirements(root: Path) -> str:
    in_gym_checkout = (root / "pyproject.toml").is_file() and (root / "nemo_gym").is_dir()
    return "-e nemo-gym[dev] @ ../../\n" if in_gym_checkout else "nemo-gym[dev]\n"
