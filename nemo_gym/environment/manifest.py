...
SPDX_LICENSE_SCHEMA_ID = "spdx-license-identifier.schema.json"
...
_SCHEMA_DIRECTORY = resources.files(__package__).joinpath("schemas")
with _SCHEMA_DIRECTORY.joinpath("spdx-license-identifier.schema.json").open(encoding="utf-8") as _stream:
    _SPDX_LICENSE_VALUES = frozenset(json.load(_stream)["enum"])
_LICENSE_VALUES = _SPDX_LICENSE_VALUES | _PRIVATE_LICENSE_VALUES
...
SemanticVersion = Annotated[str, StringConstraints(strip_whitespace=True, pattern=SEMANTIC_VERSION_PATTERN)]
...
class EnvironmentManifest(_ManifestModel):
    ...
    name: ManifestName
    version: SemanticVersion = Field(description="Semantic version of the resolved composition.")
    ...
    licensing: License = Field(
        default="unknown",
        json_schema_extra={"format": "spdx-license-identifier"},
    )
    ...
    def validate_licensing(cls, value: str) -> str:
        if value not in _LICENSE_VALUES:
            raise ValueError("licensing must be an SPDX license identifier, internal, proprietary, or unknown")
        return value
...
def manifest_json_schema() -> dict[str, Any]:
    ...
    licensing = schema["properties"]["licensing"]
    licensing.pop("format")
    licensing["anyOf"] = [
        {"$ref": SPDX_LICENSE_SCHEMA_ID},
        {"enum": sorted(_PRIVATE_LICENSE_VALUES)},
    ]
    return {
