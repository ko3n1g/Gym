...
def _asset_config_path(flag: str, value: str) -> str:
    ...
    if "/" in value and flag in {"benchmark", "environment"}:
        from nemo_gym.registry import RegistryError, resolve_catalog_entry

        try:
            catalog_entry = resolve_catalog_entry(value, flag)
        except RegistryError as error:
            if not str(error).startswith("Unknown catalog entry"):
                raise ValueError(str(error)) from error
        if catalog_entry is not None and catalog_entry.manifest_path is not None:
            return str(catalog_entry.config_path.resolve())
